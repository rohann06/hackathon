                ...
                if (address && aptosClient && asset ? .storage ? .ipfs ? .nftMetadata ? .url) {
                    ...
                    if (json.tokenName) {
                        ...
                        const result = await aptosClient.waitForTransactionWithResult(
                            aptosResponse.hash, { checkSuccess: true }
                        );
        ...
        return ( <
            ...
            div > {
                ...
                div className = " h-60 bg-black text-white w-[30%] rounded-xl" > /*{
                        idParsed && ( <
                            Player title = { idParsed.id }
                            src = { url }
                            autoPlay muted autoUrlUpload = {
                                {
                                    fallback: true,
                        ...
                        address && updateStatus === "success" ? ( <
                            ...
                            div className = " mt-14 flex justify-center items-center" > {
                                status === "success" ? ( <
                                    ...
                                    className = " bg-blue-600 text-white rounded-xl font-semibold text-[14px] lg:text-[20px] px-8 lg:px-20 py-2 " > { updateStatus === "loading" ? "Loading...." : "Upload to IPFS" } <
                                    /button>
                                ) : ( <
                                    button disabled = { status === "loading" || !video }
                                    onClick = {
                                        () => createAsset ? .()
                                    }
                                    className = " bg-blue-600 text-white rounded-xl font-semibold text-[14px] lg:text-[20px] px-8 lg:px-20 py-2 " > { status === "loading" ? "Loading..." : "Create" } <
                                    /button>
                                )
                            } <
                            /div>
